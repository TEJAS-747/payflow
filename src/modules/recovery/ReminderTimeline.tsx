...
export const ReminderTimeline: React.FC = () => {
  ...
  if (reminderLogs.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 text-center text-slate-400 dark:text-slate-500 text-xs">
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm transition-colors">
      <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100 dark:border-slate-800">
        ...
        <span className="text-[11px] text-slate-400 dark:text-slate-500">Live Delivery Log</span>
      </div>

      <div className="space-y-4">
        {reminderLogs.map((log) => (
          <div key={log.id} className="relative pl-6 border-l-2 border-amber-200 dark:border-amber-700 space-y-1">
            ...
            <div className="flex flex-wrap items-center justify-between gap-1">
              <div className="flex items-center space-x-2">
                <span className="font-bold text-xs text-slate-800 dark:text-slate-200">{log.clientName}</span>
                ...
                <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300">
                  {log.tone}
                </span>
              </div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 font-mono bg-slate-50 dark:bg-slate-800/80 p-2 rounded-lg border border-slate-100 dark:border-slate-700">
              "{log.content}"
            </p>

            <div className="flex items-center space-x-2 text-[10px] text-slate-400 dark:text-slate-500 pt-0.5">
